...
public class FastHiveDecimalImpl extends FastHiveDecimal {
    ...
    private static final BigInteger BIG_INTEGER_UNSIGNED_BYTE_MAX_VALUE = BIG_INTEGER_TWO.pow(Byte.SIZE)
            .subtract(BigInteger.ONE);
    private static final BigInteger BIG_INTEGER_UNSIGNED_SHORT_MAX_VALUE = BIG_INTEGER_TWO.pow(Short.SIZE)
            .subtract(BigInteger.ONE);
    private static final BigInteger BIG_INTEGER_UNSIGNED_INT_MAX_VALUE = BIG_INTEGER_TWO.pow(Integer.SIZE)
            .subtract(BigInteger.ONE);
    private static final BigInteger BIG_INTEGER_UNSIGNED_LONG_MAX_VALUE = BIG_INTEGER_TWO.pow(Long.SIZE)
            .subtract(BigInteger.ONE);
    ...
    private static void doRaiseSetFromBytesInvalid(
        ...
        throw new RuntimeException(
                "Invalid fast decimal \"" +
                        ...
                        " fastSignum " + fastResult.fastSignum + " fast0 " + fastResult.fast0 + " fast1 "
                        + fastResult.fast1 + " fast2 " + fastResult.fast2 +
                        " fastIntegerDigitCount " + fastResult.fastIntegerDigitCount + " fastScale "
    ...
    */
    ...
    public static boolean fastScaleDownNoRound(
        ...
        if (scaleDown < 1 || scaleDown >= THREE_X_LONGWORD_DECIMAL_DIGITS - 1) {
            throw new IllegalArgumentException(
                    "Expecting scaleDown > 0 and scaleDown < 3*16 - 1 (scaleDown " + scaleDown + ")");
        }
    ...
    public static boolean fastRoundFractionalUp(
        ...
        if (scaleDown < 1 || scaleDown > HiveDecimal.MAX_SCALE) {
            throw new IllegalArgumentException(
                    "Expecting scaleDown > 0 and scaleDown < " + HiveDecimal.MAX_SCALE + " (scaleDown " + scaleDown
                            + ")");
        }
    ...
    public static void fastRoundFractionalDown(
        ...
        if (scaleDown < 1 || scaleDown > HiveDecimal.MAX_SCALE) {
            throw new IllegalArgumentException(
                    "Expecting scaleDown > 0 and scaleDown < 38 (scaleDown " + scaleDown + ")");
        }
    ...
    public static boolean fastSubtract(
        ...
        if (leftSignum == 0) {
            fastResult.fastSet(flippedDecSignum, rightFast0, rightFast1, rightFast2, rightIntegerDigitCount,
                    rightScale);
            return true;
        }
        ...
        if (fastSignum == 0) {
            ...
            if (!isValid) {
                ...
                if (fastScale < 0 || fastScale > HiveDecimal.MAX_SCALE) {
                    ...
                    System.out.println(
                            "FAST_IS_VALID exceeds max precision: fastIntegerDigitCount " + fastIntegerDigitCount
                                    + " and fastScale " + fastScale);
                    isValid = false;
                } else {
