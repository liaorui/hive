...
class SchemaToTypeInfo {
    ...
    public static TypeInfo generateTypeInfo(Schema schema,
        ...
        if (type == BYTES && AvroSerDe.DECIMAL_TYPE_NAME
            ...
            int scale = 0;
            try {
                precision = mapper.valueToTree(schema.getObjectProp(AvroSerDe.AVRO_PROP_PRECISION)).asInt();
                scale = mapper.valueToTree(schema.getObjectProp(AvroSerDe.AVRO_PROP_SCALE)).asInt();
            } catch (Exception ex) {
        ...
        if (type == STRING &&
            ...
            int maxLength = 0;
            try {
                maxLength = mapper.valueToTree(schema.getObjectProp(AvroSerDe.AVRO_PROP_MAX_LENGTH)).asInt();
            } catch (Exception ex) {
